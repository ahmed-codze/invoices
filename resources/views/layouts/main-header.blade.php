			...
			<div class="main-header sticky side-header nav nav-item">
				<div class="container-fluid">
					<div class="main-header-left ">
						...
						<form action="/Search_invoices" class="form-group main-header-center mr-3 d-sm-none d-md-none d-lg-block"  method="POST" autocomplete="off">
							@csrf
							<input name="rdio" value="2" type="hidden">
							<input class="form-control" placeholder="ابحث برقم الفاتورة..." type="search"> <button class="btn"><i class="fas fa-search d-none d-md-block"></i></button>
						</form>
					...
					<div class="main-header-right">
						...
						<div class="nav nav-item  navbar-nav-right ml-auto">
							...
							<div class="dropdown nav-item main-header-notification">
								...
								<span class="{{ Auth::user()->unreadNotifications->count() > 0  ? 'pulse' : ''  }}"></span>
								...
								@if (Auth::user()->unreadNotifications->count() > 0) 
										<div class="d-flex">
											<h6 class="dropdown-title mb-1 tx-15 text-white font-weight-semibold">الإشعارات</h6>
												<form class="mr-auto my-auto float-left" action="markAsRead" method="POST">
													@csrf
												<button class="badge badge-pill badge-warning " type="submit">تعيين الكل كمقروء</button>
											</form>
										</div>
										<p class="dropdown-title-text subtext mb-0 text-white op-6 pb-0 tx-12 ">لديك {{Auth::user()->unreadNotifications->count()}} غير مقروءة</p>
									</div>
									...
									<a class="d-flex p-3 border-bottom" href="{{ url('InvoicesDetails') }}/{{ $notification->data['id'] }}">
										...
										<div class="mr-3">
											<h5 class="notification-label mb-1">{{$notification->data['title']}} بواسطة {{$notification->data['user']}}</h5>
											<div class="notification-subtext">{{$notification->created_at->format('Y-m-d')}}</div>
										</div>
										<div class="mr-auto" >
									...
									@endforeach
									@else 
									<p class="dropdown-title-text subtext mb-0 text-white op-6 pb-0 tx-12 ">ليس لديك رسائل جديدة</p>
